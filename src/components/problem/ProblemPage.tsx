...
const ProblemPage = () => {
  ...
  const [problemSolvers, setProblemSolvers] = useState(new Map<string, Set<string>>()); // problemId -> Set of handles
  ...
  useEffect(() => {
    ...
    let solvers = new Map<string, Set<string>>();
    ...
    for (let submission of submissions) {
      ...
      if (submission.verdict === Verdict.OK) {
        solv.add(problemId);
        // Track who solved this problem
        if (handle) {
          if (!solvers.has(problemId)) solvers.set(problemId, new Set());
          solvers.get(problemId)!.add(handle);
        }
      } else {
        att.add(problemId);
      }
