...
function Menu() {
  ...
  const submitUser = () => {
    const handles = handle.split(",").map(h => h.trim()).filter(h => h.length > 0);
    if (handles.length === 0) {
      showGeneralToast("Please enter at least one handle");
      return;
    }
    showGeneralToast(`Tracking ${handles.length} handle${handles.length > 1 ? 's' : ''}: ${handles.join(", ")}`);
      ...
      <div className="container p-0">
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="ms-auto mt-2 mt-lg-0">
            ...
            <li className="nav-item">
              <form
                className="form-inline d-flex my-2 my-lg-0 nav-item flex-column"
                onSubmit={(e) => {
                  e.preventDefault();
                  submitUser();
                }}
              >
                <input
                  ...
                  aria-label="handles"
                  title="Enter multiple CF handles separated by commas"
                  value={handle}
                  onChange={(e) => setHandle(e.target.value)}
                  style={{ minWidth: "180px" }}
                />
                {state.userList.handles.length > 0 && (
                  <div className="d-flex flex-wrap gap-1 mt-1" style={{ maxWidth: "200px" }}>
                    {state.userList.handles.map((h, idx) => {
                      const isDark = theme.themeType === ThemesType.DARK;
                      const bgColor = getUserColor(idx, isDark);
                      return (
                        <span
                          key={idx}
                          className="badge"
                          ...
                          title={`Tracking: ${h} (click to remove)`}
                          onClick={() => {
                            const newHandles = state.userList.handles.filter((_, i) => i !== idx);
                            setHandle(newHandles.join(", "));
                            fetchUsers(dispatch, newHandles.join(","));
                          }}
                        >
                          {h} ×
                        </span>
